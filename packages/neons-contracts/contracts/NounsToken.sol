...
contract NounsToken is INounsToken, Ownable, ERC721Checkpointable {
    ...
    uint256 private constant MAX_NEONS = 5555;
    uint96 public constant FEE_NUMERATOR = 690; // 690 / 10,000 = 6.9% royalty
    ...
    constructor(
        ...
        _setDefaultRoyalty(_noundersDAO, FEE_NUMERATOR);
    }
    ...
    function mint() public override onlyMinter returns (uint256) {
        require(_currentNounId < MAX_NEONS, 'NeonsToken: Max minted');
    ...
    function _mintTo(address to, uint256 nounId) internal returns (uint256) {
        ...
        if (nounId == 1) {
            // Mint the genesis Neon for Scott Lewis

            seed = seeds[nounId] = INounsSeeder.Seed({
                background: uint48(0),
                body: uint48(0),
                accessory: uint48(0),
                head: uint48(0),
            ...
            seed = seeds[nounId] = seeder.generateSeed(nounId, descriptor);
        }
