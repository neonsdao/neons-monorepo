import { ImageData, getNounData } from '@neons/assets';
import { buildSVG } from '@neons/sdk';
...
export function formatAuctionStartedTweetText(auctionId: number) {
  return `＊Incoming Transmition＊
        
 An auction has started for Neon #${auctionId}
 Learn more at https://neons.lol`;
}
...
export async function formatBidMessageText(id: number, bid: Bid) {
  ...
  return `Neon ${id} has received a bid of Ȼ${ethers.utils.formatEther(bid.amount)} from ${bidder}`;
}
...
export function getAuctionEndingSoonTweetText() {
  return `This auction is ending soon! Bid now at https://neons.lol`;
}

export function formatNewGovernanceProposalText(proposal: Proposal) {
  return `A new Neons Treasury proposal (#${proposal.id}) has been created: ${extractProposalTitle(
    proposal,
  )}`;
}

export function formatUpdatedGovernanceProposalStatusText(proposal: Proposal) {
  return `Neons Treasury proposal #${proposal.id} (${extractProposalTitle(
    proposal,
...
/**
 * Get the PNG buffer data of a Neon
 * @param tokenId The ERC721 token id
 * @returns The png buffer of the Neon or undefined
 */
export async function getNounPngBuffer(tokenId: string): Promise<Buffer | undefined> {
  const seed = await tryF(() => nounsTokenContract.seeds(tokenId));
  if (isError(seed)) {
    console.error(`Error fetching seed for token ID ${tokenId}: ${seed.message}`);
    return;
  }

  const { parts, background } = getNounData(seed);
  const svg = Buffer.from(buildSVG(parts, ImageData.palette, background));
