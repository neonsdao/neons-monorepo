...
import { CantoChain, CANTO_CHAIN_ID, CHAIN_ID, ETHERSCAN_API_KEY } from '../config';

const getBaseURL = (network: ChainId | CantoChain) => {
  switch (network) {
    case CANTO_CHAIN_ID:
      return 'https://evm.explorer.canto.io/';
    case ChainId.Goerli:
...
export const buildEtherscanHoldingsLink = (address: string): string => {
  const path = `address/${address}/coin-balances#address-tabs`;
  return new URL(path, BASE_URL).toString();
};

const getApiBaseURL = (network: ChainId | CantoChain) => {
  switch (network) {
    case CANTO_CHAIN_ID:
      return `https://api.evm.explorer.canto.io`;
    case ChainId.Goerli:
