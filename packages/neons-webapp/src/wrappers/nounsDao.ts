import { NounsDAOV2ABI, NounsDaoLogicV2Factory } from '@neons/sdk';
import {
  useBlockNumber,
  useContractCall,
  useContractCalls,
...
import { connectContractToSigner } from '../utils/connectContractToSigner';
...
const fromBlock = CHAIN_ID === CANTO_CHAIN_ID ? 2657737 : 0;
...
const getProposalState = (
  ...
  const status =
    blockNumber && blockNumber > parseInt(proposal.endBlock) && ProposalState[proposal.status] === ProposalState.PENDING
...
export const useCastRefundableVote = () => {
  ...
  return {
    castRefundableVote: async (proposalId: BigNumberish, support: BigNumberish): Promise<any> => {
      const contract = connectContractToSigner(nounsDaoContract, library);
      const gasLimit = await contract.estimateGas.castRefundableVote(proposalId, support);
      return castRefundableVote(proposalId, support, {
...
export const useCastRefundableVoteWithReason = () => {
  ...
  return {
    castRefundableVoteWithReason: async (
      ...
      return castRefundableVoteWithReason(proposalId, vote, reason, {
        gasLimit: gasLimit.add(20_000), // A 20,000 gas pad is used to avoid 'Out of gas' errors
      });
    },
